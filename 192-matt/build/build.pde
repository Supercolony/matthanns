...
void setup(){
  ...
  for (int j = 0; j < 1024; j++) {
    ...
    //Do something with the vertices, x, y, z
    for(int i=0;i<mesh.numberOfVertices();i++){
     vertices2[i][0]*=random(12.2,20)+2.2*cos(HALF_PI/100*i+HALF_PI+noise(200.5,200.5));
     vertices2[i][1]*=random(12.2,20)+2.2*sin(HALF_PI/100*i+QUARTER_PI+noise(200.5,200.5));
     vertices2[i][2]*=random(cos(frameCount*PI/100),1000)+10.2*cos(HALF_PI/500*i+QUARTER_PI+noise(20.5,1000.5));
    }
