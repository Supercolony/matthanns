...
void setup(){
  ...
  for (int j = 0; j < 1024; j++) {
    ...
    for(int i=0;i<mesh.numberOfVertices();i++){
     vertices[i][0]*=random(12.2,100)+.2*cos(HALF_PI/100*i+HALF_PI+noise(200.5,200.5));
     vertices[i][1]*=random(12.2,100)+.2*sin(HALF_PI/100*i+QUARTER_PI+noise(200.5,200.5));
     vertices[i][2]*=random(100,2000-mouseY)+1.2*cos(HALF_PI/500*i+QUARTER_PI+noise(20.5,1000.5));
    }
  ...
  void draw(){
    ...
    rotateY(frameCount*PI/500/HALF_PI*noise(20.5,1000.5));
    rotateX(frameCount*PI/500/HALF_PI*noise(20.5,1000.5));
    rotateY(frameCount*PI/500/HALF_PI*noise(20.5,1000.5));
    rotateZ(frameCount*PI/500/HALF_PI*noise(20.5,1000.5));
    fill(143,214,220,40);
    stroke(21,48,77,100);
